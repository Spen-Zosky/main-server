...
export const DIRECTORY_PATHS = {
  ...
  'stage-snapshots': './STAGE_SNAPSHOTS/'
};
...
export const serviceManagerApi = {
  ...
  async getDirectoryContents(directoryType) {
    try {
      const response = await retryRequest(
        () => apiClient.get(`/directories/${directoryType}`),
        3, // Max 3 retries
  ...
  async getFileMetadata(directoryType, fileName) {
    try {
      const response = await apiClient.get(`/files/${directoryType}/${encodeURIComponent(fileName)}/metadata`);
      return response.data;
    } catch (error) {
  ...
  async downloadFile(directoryType, fileName) {
    try {
      ...
      const downloadUrl = `${API_BASE_URL}/files/download?type=${encodeURIComponent(directoryType)}&filename=${encodeURIComponent(fileName)}`;
  ...
  async searchFiles(directoryType, query) {
    try {
      const response = await apiClient.get(`/search/${directoryType}?q=${encodeURIComponent(query)}`);
      return response.data;
    } catch (error) {
  ...
  async previewFile(directoryType, fileName) {
    try {
      const response = await apiClient.get(`/files/${directoryType}/${encodeURIComponent(fileName)}/preview`);
      return response.data;
    } catch (error) {
  ...
  async bulkDelete(directoryType, fileNames) {
    try {
      ...
      const response = await apiClient.delete('/files/bulk-delete', {
        data: { files }
      });
      return response.data;
    } catch (error) {
  ...
  async bulkDownload(directoryType, fileNames) {
    try {
      ...
      const downloadUrl = `${API_BASE_URL}/files/bulk-download`;
